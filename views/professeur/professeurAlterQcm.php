...
<div>
	<nav aria-label="breadcrumb">
		  <ol class="breadcrumb" class="bg-light" style="margin-bottom: 0px; background-color: #FAFCFC; ">
		    ...
		    <li class="breadcrumb-item"><a href="/professeur/liste_qcm">Ma bibliothèque QCM</a></li>
		    <li class="breadcrumb-item active" aria-current="<?= $titre?? 'Mon espace '?>">Modifier</li>
		  </ol>
	</nav>
	...
	<div class="container-fluid">
		<div class="d-md-flex flex-row justify-content-between">
			...
			<section class="col-md-9">
				...
				<form action="/professeur/edit_qcm" method="post" class="col-lg-12 needs-validation" novalidate>
					...
					<div class="row d-flex justify-content-between">
							...
							<div  class="card-body">
								...
								<div class="form-group row">
									...
									<?php if(isset($themes)): ?>
										    ...
										    ?>
											<option value="<?= $theme->id; ?>" <?= $select;?>><?= htmlentities($theme->libelle); ?></option>
											<?php endforeach ?>
								 	<?php endif ?>
									</select>
								...
								<div class="form-group row mb-5" id="inputTheme">
								...
								<div class="form-group row">
									...
									<input id="libelle" type="text" name="libelle" class="col-sm-8 form-control" placeholder="Entrez le libéllé de votre QCM" value="<?= $_POST['libelle']?? htmlentities($qcm->libelle); ?>" required>
								...
								<div class="form-group row">
									...
									<?php $date=date("Y-m-d"); ?>
									<input id="date_limite" type="date" name="date_limite" class="col-sm-8 form-control" value="<?= $_POST['date_limite']?? date('Y-m-d', strtotime($qcm->date_limite));?>" min="<?= $date ?>" required>
									<div class="invalid-feedback">
						...
						<div class="card p-4 col-lg-8 my-3" id="enregistrement_suite">
							...
							<div  class="card-body" id="questionReponse">
								...
								<?php foreach ($_POST['questions'] as $cle => $question): ?>
									<div class="form-group row bg-light p-3" id="">
										...
										<input type="text" name="questions[]" class="col-sm-8 form-control" value="<?= $question; ?>" required>
										<div class="invalid-feedback">
							...
							<div  class="card-body" id="questionReponse">
								...
								<?php foreach ($qcm->questions as $cle => $question): ?>
									<div class="form-group row bg-light p-3" id="">

										<label class="col-sm-4 col-form-label"><span><i class="fas fa-times" style="color: red;"></i> <strong> Question <?= $cle+1;?> : </strong></span></label>
										<input type="text" name="questions[]" class="col-sm-8 form-control" placeholder=""  value="<?= htmlentities($question->question); ?>" required>
										<div class="invalid-feedback">
								        		Le champs ne doit pas être vide !
								    	</div>
									...
									<div class="mb-5 ml-5 reponses bg-light px-5 py-3">
										...
										<div class="form-group row ">
											...
											<label class="col-sm-4 col-form-label"><span class="mr-1"><i class="fas fa-times-circle"> Option <?= $p; ?> : </i></span></label>
											<input type="text" name="reponses[<?= $cle ?>][]" class="col-sm-8 form-control" placeholder="" value="<?= htmlentities($reponse->reponse); ?>" required>
											<div class="invalid-feedback">
											    Le champs ne doit pas être vide !
											</div>	
										</div>
										<div class="form-group row ml-sm-1 d-flex justify-content-end">
											<div class="form-check form-check-inline">
													<input type="checkbox" name="valReponses[<?= $cle?>][<?= $key ?>]" value="vrai" class="form-check-input" <?= $reponse->valeur=='vrai' ? 'checked' : '' ?>>
													<label class="form-check-label ">Bonne proposition</label>
											</div>
										</div>
										</div>
								...
								<span class="cursorQuest" title="Ajouter une question" id="<?= $j; ?>"><i class="fas fa-plus-square"></i></span>
						...
						<div class="card col-lg-4 my-lg-3 bg-light">
								...
								<div class="form-group">
										...
										<input id="echelle" type="number" name="echelle_not" class=" form-control" value="<?=$_POST['echelle_not']?? htmlentities($qcm->echelle_not); ?>" min="0" max="500" required>
										<div class="invalid-feedback">
								...
								<div class="form-group">
										...
										<input id="notation_vrai" type="number" name="notation_vrai" class=" form-control" value="<?=$_POST['notation_vrai']?? htmlentities($qcm->notation_vrai); ?>" min="0" max="500" required>
										<div class="invalid-feedback">
								...
								<div class="form-group">
										...
										<input id="notation_faux" type="number" name="notation_faux" class=" form-control" value="<?=$_POST['notation_faux']?? htmlentities($qcm->notation_faux); ?>" min="-500" max="0" required>
										<div class="invalid-feedback">
								        		Le barème de notation ne doit pas être vide !
								    	</div>
								</div>
								<div class="form-group">
									...
									<div class="d-flex justify-content-start mt-1">
										<input id="jours" type="number" name="jour" class="col-5 form-control" value="<?=$_POST['jour']?? $qcm->jour; ?>" min="0" max="365" required>
										<label class="col-5 col-form-label" for="jours">Jour</label>
									</div>
									<div class="d-flex justify-content-start mt-1">
										<input id="heure" type="number" name="heure" class="col-5 form-control" value="<?=$_POST['heure']?? $qcm->heure; ?>" min="0" max="24" required>
										<label class="col-5 col-form-label" for="heure">Heure</label>
									</div>
									<div class="d-flex justify-content-start mt-1">
										<input id="minute" type="number" name="minute" class=" col-5 form-control" value="<?=$_POST['minute']?? $qcm->minute; ?>" min="0" max="60" required>
...
<script src="/js/professeur/alterQcm.js?v=<?= time() ?>"></script>
